...
export class httpRequestConfig {
  ...
  httptResponseFunction = (response: object) => {
    ...
    const statusCode: any = [this.selfconfig.config.code, '200'].includes(String(result.code));
    // 数据响应提示
...
export class axiosHttpRequest extends httpRequestConfig {
  ...
  init(options: RequestParams) {
    ...
    this.WebHttpRequest.interceptors.request.use(this.configRequestFunction, (error: any) => error);
...
export default (option: any) => {
  ...
  console.log(webAxios, 888888888);
